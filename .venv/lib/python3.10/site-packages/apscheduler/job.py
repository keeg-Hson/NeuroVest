...
class Job(object):
    ...
    def _modify(self, **changes):
        ...
        if "trigger" in changes:
            ...
            if not isinstance(trigger, BaseTrigger):
                raise TypeError(
                    "Expected a trigger instance, got %s instead" % trigger.__class__.__name__
                )
        ...
        if changes:
            raise AttributeError(
                "The following are not modifiable attributes of Job: %s" % ", ".join(changes)
            )
    ...
    def __setstate__(self, state):
        if state.get("version", 1) > 1:
            raise ValueError(
                "Job has version %s, but only version 1 can be handled" % state["version"]
            )
