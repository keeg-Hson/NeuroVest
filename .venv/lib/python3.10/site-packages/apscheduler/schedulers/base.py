...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    def add_executor(self, executor, alias="default", **executor_opts):
        ...
        with self._executors_lock:
            if alias in self._executors:
                raise ValueError(
                    'This scheduler already has an executor by the alias of "%s"' % alias
                )
    ...
    def add_jobstore(self, jobstore, alias="default", **jobstore_opts):
        ...
        with self._jobstores_lock:
            if alias in self._jobstores:
                raise ValueError(
                    'This scheduler already has a job store by the alias of "%s"' % alias
                )
    ...
    def add_job(
        ...
        **trigger_args,
        ...
        job_kwargs = dict(
            (key, value) for key, value in six.iteritems(job_kwargs) if value is not undefined
        )
        ...
        try:
            ...
            if alias in plugin_container:
                ...
                if not issubclass(plugin_cls, base_class):
                    raise TypeError(
                        "The {0} entry point does not point to a {0} class".format(type_)
                    )
            else:
    ...
    def _create_trigger(self, trigger, trigger_args):
        if isinstance(trigger, BaseTrigger):
            ...
            raise TypeError(
                "Expected a trigger instance or string, got %s instead" % trigger.__class__.__name__
            )
            ...
            for jobstore_alias, jobstore in six.iteritems(self._jobstores):
                try:
                    ...
                    self._logger.warning(
                        "Error getting due jobs from job store %r: %s", jobstore_alias, e
                    )
                    ...
                    if run_times:
                        try:
                            ...
                            event = JobSubmissionEvent(
                                EVENT_JOB_MAX_INSTANCES, job.id, jobstore_alias, run_times
                            )
                            events.append(event)
                        except BaseException:
                            self._logger.exception(
                                'Error submitting job "%s" to executor "%s"', job, job.executor
                            )
                        else:
                            event = JobSubmissionEvent(
