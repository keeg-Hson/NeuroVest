...
def pandas_cat_null(data: DataFrame) -> DataFrame:
    ...
    if cat_columns:
        ...
        transformed[cat_columns] = (
            transformed[cat_columns].apply(cat_codes).astype(np.float32).replace(-1.0, np.NaN)
        )
    if nul_columns:
...
def _from_arrow(
    ...
    if not all(pa.types.is_integer(t) or pa.types.is_floating(t) for t in data.schema.types):
        raise ValueError("Features in dataset can only be integers or floating point number")
    if enable_categorical:
...
def _transform_cudf_df(
    ...
    if not all(
        dtype.name in _pandas_dtype_mapper or (is_categorical_dtype(dtype) and enable_categorical)
        for dtype in dtypes
    ):
        _invalid_dataframe_dtype(data)
...
def _is_cupy_array(data: DataType) -> bool:
    return any(
        lazy_isinstance(data, n, "ndarray") for n in ("cupy.core.core", "cupy", "cupy._core.core")
    )
...
def _from_cupy_array(
    ...
    config = bytes(json.dumps({"missing": missing, "nthread": nthread}), "utf-8")
    _check_call(
        _LIB.XGDMatrixCreateFromCudaArrayInterface(interface_str, config, ctypes.byref(handle))
    )
    return handle, feature_names, feature_types
