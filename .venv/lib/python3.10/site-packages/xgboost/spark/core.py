...
Pred = namedtuple("Pred", ("prediction", "raw_prediction", "probability", "pred_contrib"))
...
class _SparkXGBParams(
    ...
    def _validate_params(self) -> None:
        ...
        if init_model is not None and not isinstance(init_model, Booster):
            raise ValueError(
                "The xgb_model param must be set with a `xgboost.core.Booster` " "instance."
            )
        ...
        if self.getOrDefault(eval_metric) is not None:
            if not (
                ...
                or (
                    ...
                    and all(isinstance(metric, str) for metric in self.getOrDefault(eval_metric))
                )
            ):
                raise ValueError(
...
def _get_unwrapped_vec_cols(feature_col: Column) -> List[Column]:
    ...
    return [
        ...
        features_unwrapped_vec_col.values.cast(ArrayType(FloatType())).alias("featureVectorValues"),
    ]
    ...
    def setParams(self, **kwargs: Any) -> None:  # pylint: disable=invalid-name
        ...
        for k, v in kwargs.items():
            ...
            if k in _pyspark_param_alias_map:
                if k == _inverse_pyspark_param_alias_map[self.featuresCol.name] and isinstance(
                    v, list
                ):
...
class _SparkXGBModel(Model, _SparkXGBParams, MLReadable, MLWritable):
    ...
    def _gpu_transform(self) -> bool:
        ...
        gpu_per_task = (
            _get_spark_session().sparkContext.getConf().get("spark.task.resource.gpu.amount")
        )
...
class SparkXGBModelWriter(MLWriter):
    ...
    def saveImpl(self, path: str) -> None:
        ...
        _get_spark_session().sparkContext.parallelize([booster], 1).saveAsTextFile(model_save_path)
