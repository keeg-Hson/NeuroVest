...
def _xgb_get_client(client: Optional["distributed.Client"]) -> "distributed.Client":
    ...
    if not isinstance(client, (type(distributed.get_client()), type(None))):
        raise TypeError(_expect([type(distributed.get_client()), type(None)], type(client)))
    ...
    for addr in workers:
        ...
        for ref in refs:
            if isinstance(ref, DaskDMatrix):
                ...
                args.append(ref)
        fut = client.submit(func, *args, pure=False, workers=[addr], allow_other_workers=False)
        futures.append(fut)
...
def _maybe_dataframe(data: Any, prediction: Any, columns: List[int], is_df: bool) -> Any:
    ...
    if _can_output_df(is_df, prediction.shape):
        ...
        if lazy_isinstance(data, "cudf.core.dataframe", "DataFrame"):
            ...
            prediction = DataFrame(prediction, columns=columns, dtype=numpy.float32, index=index)
    return prediction
...
async def _direct_predict_impl(  # pylint: disable=too-many-branches
    ...
    if len(output_shape) >= 3 and isinstance(data, dd.DataFrame):
        ...
        # binding.
        raise ValueError("Use `da.Array` or `DaskDMatrix` when output has more than 2 dimensions.")
    if _can_output_df(isinstance(data, dd.DataFrame), output_shape):
        ...
        if base_margin is not None and isinstance(base_margin, (dd.Series, dd.DataFrame)):
...
async def _get_model_future(
    ...
    if isinstance(model, Booster):
        ...
        t = booster.type
        if t is not Booster:
            raise TypeError(f"Underlying type of model future should be `Booster`, got {t}")
    else:
...
async def _predict_async(
    ...
    if isinstance(data, (da.Array, dd.DataFrame)):
        ...
        return await _direct_predict_impl(mapped_predict, _booster, data, None, _output_shape, meta)
    ...
    for i, rows in enumerate(all_shapes):
        arrays.append(
            da.from_delayed(futures[i], shape=(rows,) + output_shape[1:], dtype=numpy.float32)
        )
...
async def _inplace_predict_async(  # pylint: disable=too-many-branches
    ...
    if base_margin is not None and not isinstance(data, (da.Array, dd.DataFrame, dd.Series)):
    ...
    return await _direct_predict_impl(mapped_predict, booster, data, base_margin, shape, meta)
    ...
    **kwargs: Any,
    ...
    def _dispatch(ref: Optional[DaskDMatrix], **kwargs: Any) -> DaskDMatrix:
        if _can_use_qdm(tree_method):
            return DaskQuantileDMatrix(client=client, ref=ref, max_bin=max_bin, **kwargs)
...
@contextmanager
def _set_worker_client(model: "DaskScikitLearnBase", client: "distributed.Client") -> Generator:
