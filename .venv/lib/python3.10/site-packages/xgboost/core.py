...
def from_pystr_to_cstr(data: Union[str, List[str]]) -> Union[bytes, ctypes.Array]:
    ...
    if isinstance(data, list):
        ...
        pointers: ctypes.Array[ctypes.c_char_p] = (ctypes.c_char_p * len(data_as_bytes))(
            *data_as_bytes
        )
        return pointers
    raise TypeError()
...
def c_array(ctype: Type[CTypeT], values: ArrayLike) -> Union[ctypes.Array, ctypes._Pointer]:
...
class DataIter(ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, cache_prefix: Optional[str] = None, release_data: bool = True) -> None:
    ...
    def throw_if(func: Callable[..., _T]) -> Callable[..., _T]:
        ...
        def inner_f(*args: Any, **kwargs: Any) -> _T:
            ...
            if extra_args > 0:
                ...
                args_msg = [
                    f"{name}" for name, _ in zip(kwonly_args[:extra_args], args[-extra_args:])
                ]
...
class DMatrix:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def set_info(
        ...
        *,
        ...
        if feature_weights is not None:
            dispatch_meta_backend(matrix=self, data=feature_weights, name="feature_weights")
    ...
    def save_binary(self, fname: Union[str, os.PathLike], silent: bool = True) -> None:
        ...
        _check_call(_LIB.XGDMatrixSaveBinary(self.handle, c_str(fname), ctypes.c_int(silent)))
    ...
    def get_data(self) -> scipy.sparse.csr_matrix:
        ...
        _check_call(_LIB.XGDMatrixGetDataAsCSR(self.handle, config, c_indptr, c_indices, c_data))
    ...
    def slice(self, rindex: Union[List[int], np.ndarray], allow_groups: bool = False) -> "DMatrix":
    ...
    def feature_names(self, feature_names: Optional[FeatureNames]) -> None:
        ...
        feature_names = _validate_feature_info(feature_names, self.num_col(), "feature names")
        if len(feature_names) != len(set(feature_names)):
            ...
            raise ValueError(f"feature_names must be unique. Duplicates found: {duplicates}")

        # prohibit the use symbols that may affect parsing. e.g. []<
        if not all(
            isinstance(f, str) and not any(x in f for x in ["[", "]", "<"]) for f in feature_names
        ):
        ...
        c_feature_names = (ctypes.c_char_p * len(feature_names_bytes))(*feature_names_bytes)
        _check_call(
            _LIB.XGDMatrixSetStrFeatureInfo(
                self.handle,
                c_str("feature_name"),
    ...
    def feature_types(self, feature_types: Optional[FeatureTypes]) -> None:
        ...
        feature_types = _validate_feature_info(feature_types, self.num_col(), "feature types")

        feature_types_bytes = [bytes(f, encoding="utf-8") for f in feature_types]
        c_feature_types = (ctypes.c_char_p * len(feature_types_bytes))(*feature_types_bytes)
        _check_call(
            _LIB.XGDMatrixSetStrFeatureInfo(
                self.handle,
                c_str("feature_type"),
...
class Booster:
    ...
    def __init__(
        ...
        _check_call(_LIB.XGBoosterCreate(dmats, c_bst_ulong(len(cache)), ctypes.byref(self.handle)))
        for d in cache:
    ...
    def attr(self, key: str) -> Optional[str]:
        ...
        _check_call(
            _LIB.XGBoosterGetAttr(self.handle, c_str(key), ctypes.byref(ret), ctypes.byref(success))
        )
        if success.value != 0:
            value = ret.value
            assert value
    ...
    def attributes(self) -> Dict[str, Optional[str]]:
        ...
        sarr = ctypes.POINTER(ctypes.c_char_p)()
        _check_call(
            _LIB.XGBoosterGetAttrNames(self.handle, ctypes.byref(length), ctypes.byref(sarr))
        )
    ...
    def _set_feature_info(self, features: Optional[FeatureInfo], field: str) -> None:
        if features is not None:
            ...
            _check_call(
                _LIB.XGBoosterSetStrFeatureInfo(self.handle, c_str(field), None, c_bst_ulong(0))
            )
    ...
    def update(self, dtrain: DMatrix, iteration: int, fobj: Optional[Objective] = None) -> None:
        ...
        if fobj is None:
            _check_call(
                _LIB.XGBoosterUpdateOneIter(self.handle, ctypes.c_int(iteration), dtrain.handle)
            )
        else:
    ...
    def inplace_predict(
        ...
        if validate_features:
            if not hasattr(data, "shape"):
                raise TypeError("`shape` attribute is required when `validate_features` is True.")
        ...
        if _is_cudf_df(data):
            ...
            data, cat_codes, fns, _ = _transform_cudf_df(data, None, None, enable_categorical)
            interfaces_str = _cudf_array_interfaces(data, cat_codes)
            if validate_features:
                self._validate_features(fns)
            _check_call(
        ...
        raise TypeError("Data type:" + str(type(data)) + " not supported by inplace prediction.")
    ...
    def _validate_features(self, feature_names: Optional[FeatureNames]) -> None:
        ...
        if feature_names is None and self.feature_names is not None:
            raise ValueError(
                "training data did not have the following fields: " + ", ".join(self.feature_names)
            )
        ...
        if self.feature_names != feature_names:
            ...
            if my_missing:
                msg += "\ntraining data did not have the following fields: " + ", ".join(
                    str(s) for s in my_missing
                )
    ...
    def get_split_value_histogram(
        ...
        if nph_stacked.size == 0:
            ...
            if feature_t == "c":  # categorical
                raise ValueError("Split value historgam doesn't support categorical split.")
