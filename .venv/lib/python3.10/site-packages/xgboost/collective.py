...
def broadcast(data: _T, root: int) -> _T:
    ...
    # run first broadcast
    _check_call(
        _LIB.XGCommunicatorBroadcast(ctypes.byref(length), ctypes.sizeof(ctypes.c_ulong), root)
    )
    if root != rank:
        dptr = (ctypes.c_char * length.value)()
        # run second
        _check_call(
            _LIB.XGCommunicatorBroadcast(ctypes.cast(dptr, ctypes.c_void_p), length.value, root)
        )
