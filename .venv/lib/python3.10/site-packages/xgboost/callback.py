...
class LearningRateScheduler(TrainingCallback):
    ...
    def __init__(self, learning_rates: Union[Callable[[int], float], Sequence[float]]) -> None:
...
class EarlyStopping(TrainingCallback):
    ...
    def after_iteration(
        ...
        if not isinstance(data_name, str):
            raise TypeError(f"The name of the dataset should be a string. Got: {type(data_name)}")
    ...
    def after_training(self, model: _Model) -> _Model:
        ...
        try:
            ...
            raise XGBoostError("`save_best` is not applicable to the current booster") from e
...
class TrainingCheckPoint(TrainingCallback):
    ...
    def after_iteration(
        ...
        if self._epoch == self._iterations:
            path = os.path.join(
                self._path,
                self._name + "_" + str(epoch) + (".pkl" if self._as_pickle else ".json"),
            )
            self._epoch = 0
