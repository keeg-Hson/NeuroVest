...
def check_init_estimation(tree_method: str) -> None:
    ...
    run_clf(X, y)
    X, y = make_multilabel_classification(n_samples=4096, n_labels=3, n_classes=5, random_state=17)
    run_clf(X, y)
...
def check_cut(n_entries: int, indptr: np.ndarray, data: np.ndarray, dtypes: Any) -> None:
