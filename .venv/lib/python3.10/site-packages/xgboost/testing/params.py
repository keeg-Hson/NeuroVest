...
hist_multi_parameter_strategy = strategies.fixed_dictionaries(
    {
        ...
        "max_leaves": strategies.integers(0, 1024),
        "max_bin": strategies.integers(2, 512),
        "multi_strategy": strategies.sampled_from(["multi_output_tree", "one_output_per_tree"]),
        "grow_policy": strategies.sampled_from(["lossguide", "depthwise"]),
        "min_child_weight": strategies.floats(0.5, 2.0),
        # We cannot enable subsampling as the training loss can increase
        # 'subsample': strategies.floats(0.5, 1.0),
...
lambdarank_parameter_strategy = strategies.fixed_dictionaries(
    {
        ...
        "objective": strategies.sampled_from(["rank:ndcg", "rank:map", "rank:pairwise"]),
    }
).filter(lambda x: not (x["lambdarank_unbiased"] and x["lambdarank_pair_method"] == "mean"))
