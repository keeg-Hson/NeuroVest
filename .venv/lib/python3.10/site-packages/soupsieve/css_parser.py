...
RE_NTH = re.compile(
    rf"(?P<s1>[-+])?(?P<a>[0-9]+n?|n)(?:(?<=n){WSC}*(?P<s2>[-+]){WSC}*(?P<b>[0-9]+))?", re.I
)
...
class SpecialPseudoPattern(SelectorPattern):
    ...
    def __init__(
        self, patterns: tuple[tuple[str, tuple[str, ...], str, type[SelectorPattern]], ...]
    ) -> None:
        """Initialize."""
...
class CSSParser:
    ...
    def __init__(
        self, selector: str, custom: dict[str, str | ct.SelectorList] | None = None, flags: int = 0
    ) -> None:
        """Initialize."""
