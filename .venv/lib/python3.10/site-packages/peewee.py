...
class CTE(_HashableSource, Source):
    ...
    def select_from(self, *columns):
        if not columns:
            raise ValueError(
                "select_from() must specify one or more columns " "from the CTE to select."
            )
...
class ColumnBase(Node):
    ...
    def _escape_like_expr(self, s, template):
        if s.find("_") >= 0 or s.find("%") >= 0 or s.find("\\") >= 0:
            ...
            return NodeList(
                (Value(template % s, converter=False), SQL("ESCAPE"), Value("\\", converter=False))
            )
        return template % s
...
class Ordering(WrappedNode):
    def __init__(self, node, direction, collation=None, nulls=None):
        ...
        if nulls and nulls.lower() not in ("first", "last"):
            raise ValueError(
                'Ordering nulls= parameter must be "first" or ' '"last", got: %s' % nulls
            )
    ...
    def __sql__(self, ctx):
        ...
        if not len(self.arguments):
            ...
            with ctx(in_function=True, function_arg_count=len(self.arguments)):
                ctx.sql(
                    EnclosedNodeList(
                        [(arg if isinstance(arg, Node) else Value(arg, False)) for arg in args]
                    )
                )
...
class OnConflict(Node):
    def __init__(
        ...
        if conflict_target is not None and conflict_constraint is not None:
            raise ValueError(
                'only one of "conflict_target" and ' '"conflict_constraint" may be specified.'
            )
    ...
    def update(self, _data=None, **kwargs):
        if _data and kwargs and not isinstance(_data, dict):
            raise ValueError(
                "Cannot mix data with keyword arguments in the " "OnConflict update method."
            )
        _data = _data or {}
        if kwargs:
            _data.update(kwargs)
    ...
    def inner(self, database=None, *args, **kwargs):
        ...
        if not database:
            raise InterfaceError(
                "Query must be bound to a database in order " 'to call "%s".' % method.__name__
            )
...
class Insert(_WriteQuery):
    ...
    def _generate_insert(self, insert, ctx):
        ...
        columns_converters = [
            (column, column.db_value if isinstance(column, Field) else None) for column in columns
        ]
...
class ModelIndex(Index):
    ...
    def _generate_name_from_fields(self, model, fields):
        ...
        if not accum:
            raise ValueError(
                "Unable to generate a name for the index, please " "explicitly specify a name."
            )
...
class SqliteDatabase(Database):
    ...
    def _connect(self):
        ...
        conn = sqlite3.connect(
            self.database, timeout=self._timeout, isolation_level=None, **self.connect_params
        )
        try:
    ...
    def get_views(self, schema=None):
        sql = ('SELECT name, sql FROM "%s".sqlite_master WHERE type=? ' "ORDER BY name") % (
            schema or "main"
        )
    ...
    def get_indexes(self, table, schema=None):
        ...
        return [
            IndexMetadata(
                name, index_to_sql[name], index_columns[name], name in unique_indexes, table
            )
    ...
    def get_columns(self, table, schema=None):
        ...
        return [
            ColumnMetadata(r[1], r[2], not r[3], bool(r[5]), table, r[4]) for r in cursor.fetchall()
        ]
    ...
    def conflict_update(self, oc, query):
        ...
        if action == "nothing":
            ...
            raise ValueError(
                "SQLite does not support specifying named " "constraints for conflict resolution."
            )
        elif not oc._conflict_target:
            raise ValueError(
                "SQLite requires that a conflict target be " "specified when doing an upsert."
            )
...
class PostgresqlDatabase(Database):
    ...
    def get_views(self, schema=None):
        ...
        cursor = self.execute_sql(query, (schema or "public",))
        return [
            ViewMetadata(view_name, sql.strip(" \t;")) for (view_name, sql) in cursor.fetchall()
        ]
    ...
    def conflict_update(self, oc, query):
        ...
        if action in ("ignore", "nothing"):
            ...
            raise ValueError(
                "Postgres requires that a conflict target be " "specified when doing an upsert."
            )
...
class Field(ColumnBase):
    ...
    def __init__(
        ...
        if db_column is not None:
            __deprecated__(
                '"db_column" has been deprecated in favor of ' '"column_name" for Field objects.'
            )
...
class DecimalField(Field):
    ...
    def __init__(
        self, max_digits=10, decimal_places=5, auto_round=False, rounding=None, *args, **kwargs
    ):
...
class BigBitFieldAccessor(FieldAccessor):
    ...
    def __set__(self, instance, value):
        if isinstance(value, memoryview):
            ...
            raise ValueError(
                "Value must be either a bytes, memoryview or " "BigBitFieldData instance."
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if to_field is not None:
            __deprecated__(
                '"to_field" has been deprecated in favor of ' '"field" for ForeignKeyField objects.'
            )
            field = to_field
        if related_name is not None:
            __deprecated__(
                '"related_name" has been deprecated in favor of ' '"backref" for Field objects.'
            )
    ...
    def __init__(
        ...
        if through_model is not None:
            ...
            if not _is_backref and (on_delete is not None or on_update is not None):
                raise ValueError(
                    "Cannot specify on_delete or on_update when " "through_model is specified."
                )
        self.rel_model = model
...
class CompositeKey(MetaField):
    ...
    def __set__(self, instance, value):
        if not isinstance(value, (list, tuple)):
            raise TypeError(
                "A list or tuple must be used to set the value of " "a composite primary key."
            )
    ...
    def __sql__(self, ctx):
        ...
        return ctx.sql(
            NodeList([self.model._meta.fields[field] for field in self.field_names], ", ", parens)
        )
...
class SchemaManager(object):
    ...
    def database(self):
        ...
        if db is None:
            raise ImproperlyConfigured(
                "database attribute does not appear to " "be set on the model: %s" % self.model
            )
        return db
    ...
    def _create_table(self, safe=True, **options):
        ...
        if meta.composite_key:
            pk_columns = [
                meta.fields[field_name].column for field_name in meta.primary_key.field_names
            ]
    ...
    def _check_sequences(self, field):
        if not field.sequence or not self.database.sequences:
            raise ValueError(
                "Sequences are either not supported, or are not " 'defined for "%s".' % field.name
            )
...
class Metadata(object):
    def __init__(
        ...
        **kwargs,
        ...
        if not table_name:
            table_name = (
                self.table_function(model) if self.table_function else self.make_table_name()
            )
    ...
    def __new__(cls, name, bases, attrs, **kwargs):
        ...
        if pk is None:
            if parent_pk is not False:
                pk, pk_name = (
                    (parent_pk, parent_pk.name) if parent_pk is not None else (AutoField(), "id")
                )
            else:
                pk = False
...
class Model(with_metaclass(ModelBase, Node)):
    ...
    def insert_from(cls, query, fields):
        columns = [
            getattr(cls, field) if isinstance(field, basestring) else field for field in fields
        ]
...
class ModelSelect(BaseModelSelect, Select):
    ...
    def _generate_on_clause(self, src, dest, to_field=None, on=None):
        ...
        if not fk_fields:
            ...
            fk_fields = [
                f for f in fk_fields if ((f is target) or (is_backref and f.rel_field is to_field))
            ]
...
class ManyToManyQuery(ModelSelect):
    ...
    def clear(self):
        src_id = getattr(self._instance, self._src_attr)
        return (
            self._accessor.through_model.delete().where(self._accessor.src_fk == src_id).execute()
        )
            ...
            if field is not None:
                ...
                if node.dest not in self.key_to_constructor:
                    raise ValueError(
                        ...
                        "among the selected sources." % (node.unwrap(), node.dest, node.dest)
                    )
                key = node.dest
            else:
...
class PrefetchQuery(
    collections.namedtuple(
        "_PrefetchQuery", ("query", "fields", "is_backref", "rel_models", "field_to_name", "model")
    )
):
    def __new__(
        cls, query, fields=None, is_backref=None, rel_models=None, field_to_name=None, model=None
    ):
        if fields:
            if is_backref:
...
def prefetch_add_subquery(sq, subqueries, prefetch_type):
    ...
    for i, subquery in enumerate(subqueries):
        ...
        for j in reversed(range(i + 1)):
            ...
            raise AttributeError(
                "Error: unable to find foreign key for " "query: %s%s" % (subquery, tgt_err)
            )
        ...
        if fks:
            if prefetch_type == PREFETCH_TYPE.WHERE:
                expr = reduce(
                    operator.or_, [(fk << last_query.select(pk)) for (fk, pk) in zip(fks, pks)]
                )
