...
class SchemaMigrator(object):
    ...
    def apply_default(self, table, column_name, field):
        ...
        return (
            ...
            .sql(Entity(table))
            .literal(" SET ")
            .sql(Expression(Entity(column_name), OP.EQ, field.db_value(default), flat=True))
        )
    ...
    def add_unique(self, table, *column_names):
        ...
        constraint = NodeList(
            (SQL("UNIQUE"), EnclosedNodeList([Entity(column) for column in column_names]))
        )
...
class CockroachDBMigrator(PostgresqlMigrator):
    ...
    def drop_index(self, table, index_name):
        return (
            self.make_context().literal("DROP INDEX ").sql(Entity(index_name)).literal(" CASCADE")
        )
