...
class Signal(object):
    ...
    def connect(self, receiver, name=None, sender=None):
        ...
        if key not in self._receivers:
            ...
            raise ValueError(
                "receiver named %s (for sender=%s) already " "connected" % (name, sender or "any")
            )
    ...
    def disconnect(self, receiver=None, name=None, sender=None):
        ...
        key = (name, sender)
        if key not in self._receivers:
            raise ValueError("receiver named %s for sender=%s not found." % (name, sender or "any"))

        self._receivers.remove(key)
        self._receiver_list = [(n, r, s) for n, r, s in self._receiver_list if (n, s) != key]
