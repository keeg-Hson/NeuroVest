...
class Column(object):
    ...
    def __repr__(self):
        attrs = ["field_class", "raw_column_type", "nullable", "primary_key", "column_name"]
...
class Metadata(object):
    ...
    def get_columns(self, table, schema=None):
        metadata = OrderedDict(
            (metadata.name, metadata) for metadata in self.database.get_columns(table, schema)
        )
...
_DatabaseMetadata = namedtuple(
    "_DatabaseMetadata", ("columns", "primary_keys", "foreign_keys", "model_names", "indexes")
)
...
class Introspector(object):
    ...
    def introspect(
        self, table_names=None, literal_column_names=False, include_views=False, snake_case=True
    ):
        ...
        def _create_model(table, models):
            ...
            for foreign_key in database.foreign_keys[table]:
                ...
                if dest not in models and dest != table:
                    if dest in pending:
                        warnings.warn(
                            "Possible reference cycle found between " "%s and %s" % (table, dest)
                        )
                    else:
...
def print_model(model, indexes=True, inline_indexes=False):
    ...
    for field in model._meta.sorted_fields:
        ...
        if isinstance(field, ForeignKeyField):
            parts.append(" FK: %s.%s" % (field.rel_model.__name__, field.rel_field.name))
        print("".join(parts))
