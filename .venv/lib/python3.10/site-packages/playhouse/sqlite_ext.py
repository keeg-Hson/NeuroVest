...
class SearchField(Field):
    def __init__(self, unindexed=False, column_name=None, **k):
        if k:
            raise ValueError(
                "SearchField does not accept these keyword " "arguments: %s." % sorted(k)
            )
    ...
    def fts_column_index(self):
        if not hasattr(self, "_fts_column_index"):
            search_fields = [
                f.name for f in self.model._meta.sorted_fields if isinstance(f, SearchField)
            ]
...
class FTSModel(BaseFTSModel):
    ...
    @classmethod
    def search(
        cls, term, weights=None, with_score=False, score_alias="score", explicit_ordering=False
    ):
...
_alphanum = (
    set('\t ,"(){}*:_+0123456789') | set(_alphabet) | set(_alphabet.upper()) | set((chr(26),))
)
...
class FTS5Model(BaseFTSModel):
    ...
    * snippet(tbl, col_idx, prefix, suffix, ?, max_tokens)
    ...
    _error_messages = {
        "field_type": (
            "Besides the implicit `rowid` column, all columns must " "be instances of SearchField"
        ),
    ...
    def search(
        ...
        return cls.search_bm25(
            FTS5Model.clean_query(term), weights, with_score, score_alias, explicit_ordering
        )
    ...
    def search_bm25(
        ...
        return (
            cls.select(*selection).where(cls.match(FTS5Model.clean_query(term))).order_by(order_by)
        )
...
class LSMTable(VirtualModel):
    ...
    def clean_options(cls, options):
        ...
        if not filename:
            raise ValueError(
                "LSM1 extension requires that you specify a " "filename for the LSM database."
            )
        else:
