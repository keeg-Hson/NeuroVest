...
class PriceHistory:
    ...
    def history(
        ...
        if start or period is None or period.lower() == "max":
            ...
            if start_user is not None:
                ...
                _price_data_debug += (
                    f'{pd.Timestamp(start, unit="s").tz_localize("UTC").tz_convert(tz).date()}'
                )
            else:
                _price_data_debug += (
            ...
            if end_user is not None:
                ...
                _price_data_debug += (
                    f'{pd.Timestamp(end, unit="s").tz_localize("UTC").tz_convert(tz).date()})'
                )
            else:
                _price_data_debug += (
        ...
        if not keepna:
            data_colnames = (
                _PRICE_COLNAMES_ + ["Volume"] + ["Dividends", "Stock Splits", "Capital Gains"]
            )
    ...
    def _reconstruct_intervals_batch(self, df, interval, prepost, tag=-1):
        ...
        logger.debug(
            f"min_dt={min_dt} interval={interval} sub_interval={sub_interval}", extra=log_extras
        )
        ...
        for g in dts_groups:
            ...
            reject = False
            if sub_interval == "1h" and (_datetime.date.today() - start_d) > _datetime.timedelta(
                days=729
            ):
                reject = True
            ...
            if interval == "1wk":
                ...
                df_fine["Week Start"] = (
                    df_fine.index.tz_localize(None).to_period("W-" + week_end_day).start_time
                )
                grp_col = "Week Start"
            elif interval == "1d":
            ...
            if not calib_filter.any():
                # Can't calibrate so don't attempt repair
                logger.info(
                    f"Can't calibrate block starting {start_d} so aborting repair", extra=log_extras
                )
                continue
    ...
    def _fix_zeroes(self, df, interval, tz_exchange, prepost):
        ...
        if f_prices_bad.sum() == len(price_cols) * len(df2):
            ...
            logger.debug(
                "No good data for calibration so cannot fix price=0 bad data", extra=log_extras
            )
            if "Repaired?" not in df.columns:
                df["Repaired?"] = False
            return df
        ...
        if n_fixed > 0:
            msg = (
                f"{self.ticker}: fixed {n_fixed}/{n_before} value=0 errors in {interval} price data"
            )
            if n_fixed < 4:
    ...
    def _fix_bad_div_adjust(self, df, interval, currency):
        ...
        for i in range(len(div_indices) - 1, -1, -1):
            ...
            if isclose(
                df2["Low"].iloc[div_idx], df2["Close"].iloc[div_idx - 1] * 100, rel_tol=0.025
            ):
                ...
                df2["Low"].iloc[div_idx], df2["Close"].iloc[div_idx - 1] * 0.01, rel_tol=0.025
            ):
        ...
        for cid in cluster_ids:
            ...
            if len(div_pcts) > 1:
                time_diffs = div_pcts["%"].index.to_series().diff().dt.total_seconds() / (
                    365.25 * 24 * 60 * 60
                )
        ...
        if "div_too_big" in checks and "div_exceeds_adj" in checks:
            ...
            for i in range(len(div_status_df)):
                ...
                if row["div_too_big"] and row["div_exceeds_adj"]:
                    ...
                    also_correct_adj = (
                        abs(ratio - (currency_divide * currency_divide)) < currency_divide
                    )
                    if also_correct_adj:
    ...
    def _fix_prices_sudden_change(
        ...
        logger.debug(
            f"start_min={start_min} change={change:.4f} (rcp={1.0/change:.4f})", extra=log_extras
        )
        ...
        if max(split, split_rcp) < 1.0 + largest_change_pct:
            logger.debug(
                "Split ratio too close to normal price volatility. Won't repair", extra=log_extras
            )
            logger.debug(
