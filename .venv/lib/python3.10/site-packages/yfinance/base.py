...
class TickerBase:
    ...
    def get_shares_full(self, start=None, end=None, proxy=_SENTINEL_):
        ...
        shares_url = (
            f"{ts_url_base}&period1={int(start.timestamp())}&period2={int(end.timestamp())}"
        )
        try:
    ...
    def _get_earnings_dates_using_scrape(self, limit=12, offset=0) -> Optional[pd.DataFrame]:
        ...
        if table:
            ...
            df["Earnings Date"] = pd.Series(
                [dt.tz_localize(tz) for dt, tz in zip(df["Earnings Date"], tzs)]
            )
    ...
    @utils.log_indent_decorator
    def _get_earnings_dates_using_screener(
        self, limit=12, proxy=_SENTINEL_
        ...
        # Extract data
        columns = [
            row["label"] for row in json_data["finance"]["result"][0]["documents"][0]["columns"]
        ]
