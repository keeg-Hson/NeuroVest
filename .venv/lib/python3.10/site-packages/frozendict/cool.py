...
def isIterableNotString(o):
    ...
    return (
        isinstance(o, abc.Iterable) and not isinstance(o, memoryview) and not hasattr(o, "isalpha")
    )
...
def register(to_convert, converter, *, inverse=False):
    ...
    try:
        ...
        raise ValueError(f"`converter` parameter must be a callable, {converter}" + "found")
    ...
    if to_convert in freeze_conversion_map:
        ...
        warnings.warn(f"{to_convert.__name__} is already in the conversion map", FreezeWarning)
...
_freeze_types = [x for x in _freeze_conversion_map] + [x for x in _freeze_conversion_inverse_map]
...
def deepfreeze(o, custom_converters=None, custom_inverse_converters=None):
    ...
    for type_i, converter in custom_converters.items():
        ...
        try:
            ...
            raise ValueError(
                f"converter for {type_i} in `custom_converters` " + "parameter is not a callable"
            )
