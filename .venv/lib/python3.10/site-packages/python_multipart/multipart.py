...
class File:
    ...
    def __init__(
        self, file_name: bytes | None, field_name: bytes | None = None, config: FileConfig = {}
    ) -> None:
                ...
                if transfer_encoding in (b"binary", b"8bit", b"7bit"):
                    ...
                    if self.config["UPLOAD_ERROR_ON_BAD_CTE"]:
                        raise FormParserError(
                            'Unknown Content-Transfer-Encoding "{!r}"'.format(transfer_encoding)
                        )
                    else:
...
def create_form_parser(
    ...
    form_parser = FormParser(
        content_type, on_field, on_file, boundary=boundary, file_name=file_name, config=config
    )
