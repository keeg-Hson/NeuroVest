...
class BaseTestCase(unittest.TestCase, metaclass=BaseTestCaseMeta):
    ...
    def tearDown(self):
        ...
        if getattr(self.loop, "_debug_cc", False):
            ...
            self.assertEqual(
                self.loop._debug_cb_handles_count, 0, "not all callbacks (call_soon) are GCed"
            )
            ...
            self.assertEqual(
                self.loop._debug_stream_write_ctx_cnt, 0, "not all stream write contexts are GCed"
            )
    ...
    def tcp_server(
        self, server_prog, *, family=socket.AF_INET, addr=None, timeout=5, backlog=1, max_clients=10
    ):
...
class TestSocketWrapper:
    ...
    def starttls(
        self, ssl_context, *, server_side=False, server_hostname=None, do_handshake_on_connect=True
    ):
