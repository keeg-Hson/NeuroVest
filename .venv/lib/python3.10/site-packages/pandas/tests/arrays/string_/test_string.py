...
def test_arrow_roundtrip(dtype, string_storage2, request, using_infer_string):
    ...
    if using_infer_string and string_storage2 != "pyarrow_numpy":
        request.applymarker(
            pytest.mark.xfail(reason="infer_string takes precedence over string storage")
        )
...
@pytest.mark.filterwarnings("ignore:Passing a BlockManager:DeprecationWarning")
def test_arrow_load_from_zero_chunks(dtype, string_storage2, request, using_infer_string):
    # GH-41040
    pa = pytest.importorskip("pyarrow")
