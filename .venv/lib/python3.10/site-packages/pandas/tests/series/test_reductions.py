...
def test_validate_sum_initial():
    ser = Series([1, 2])
    msg = r"the 'initial' parameter is not supported in the pandas implementation of sum\(\)"
    with pytest.raises(ValueError, match=msg):
...
def test_validate_stat_keepdims():
    ser = Series([1, 2])
    msg = r"the 'keepdims' parameter is not supported in the pandas implementation of sum\(\)"
    with pytest.raises(ValueError, match=msg):
