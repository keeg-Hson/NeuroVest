...
def test_error_no_prefix_contains_unassigned():
    ...
    with pytest.raises(
        ValueError,
        match=(r"Dummy DataFrame contains unassigned value\(s\); First instance in row: 2"),
    ):
        from_dummies(dummies)
...
def test_error_no_prefix_multi_assignment():
    ...
    with pytest.raises(
        ValueError,
        match=(r"Dummy DataFrame contains multi-assignment\(s\); First instance in row: 2"),
    ):
        from_dummies(dummies)
...
def test_error_with_prefix_sep_wrong_type(dummies_basic):
    with pytest.raises(
        TypeError,
        match=(r"Expected 'sep' to be of type 'str' or 'None'; Received 'sep' of type: list"),
    ):
...
def test_error_with_prefix_double_assignment():
    ...
    with pytest.raises(
        ValueError,
        match=(r"Dummy DataFrame contains multi-assignment\(s\); First instance in row: 0"),
    ):
        from_dummies(dummies, sep="_")
