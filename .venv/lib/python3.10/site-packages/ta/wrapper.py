...
def add_volatility_ta(
    ...
    # Bollinger Bands
    indicator_bb = BollingerBands(close=df[close], window=20, window_dev=2, fillna=fillna)
...
def add_trend_ta(
    ...
    # TRIX Indicator
    df[f"{colprefix}trend_trix"] = TRIXIndicator(close=df[close], window=15, fillna=fillna).trix()
...
def add_momentum_ta(
    ...
    df[f"{colprefix}momentum_rsi"] = RSIIndicator(close=df[close], window=14, fillna=fillna).rsi()
    ...
    # Rate Of Change
    df[f"{colprefix}momentum_roc"] = ROCIndicator(close=df[close], window=12, fillna=fillna).roc()
