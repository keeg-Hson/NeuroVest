...
class TSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        smoothed = (
            diff_close.ewm(span=self._window_slow, min_periods=min_periods_r, adjust=False)
            .mean()
...
class UltimateOscillator(IndicatorMixin):
    ...
    def _run(self):
        ...
        buying_pressure = self._close - pd.DataFrame({"low": self._low, "close": close_shift}).min(
            axis=1, skipna=False
        )
        ...
        self._uo = (
            100.0
            * ((self._weight1 * avg_s) + (self._weight2 * avg_m) + (self._weight3 * avg_l))
...
class StochasticOscillator(IndicatorMixin):
    ...
    def stoch_signal(self) -> pd.Series:
        ...
        stoch_d = self._stoch_k.rolling(self._smooth_window, min_periods=min_periods).mean()
...
class ROCIndicator(IndicatorMixin):
    ...
    def _run(self):
        self._roc = (
            (self._close - self._close.shift(self._window)) / self._close.shift(self._window)
        ) * 100
...
class StochRSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        self._rsi = RSIIndicator(close=self._close, window=self._window, fillna=self._fillna).rsi()
...
def stoch_signal(high, low, close, window=14, smooth_window=3, fillna=False) -> pd.Series:
...
def williams_r(high, low, close, lbp=14, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return WilliamsRIndicator(high=high, low=low, close=close, lbp=lbp, fillna=fillna).williams_r()
...
def kama(close, window=10, pow1=2, pow2=30, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return KAMAIndicator(close=close, window=window, pow1=pow1, pow2=pow2, fillna=fillna).kama()
