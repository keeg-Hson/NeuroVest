...
class WMAIndicator(IndicatorMixin):
    ...
    def _run(self):
        _weight = pd.Series(
            [i * 2 / (self._window * (self._window + 1)) for i in range(1, self._window + 1)]
        )
        ...
        self._wma = self._close.rolling(self._window).apply(weighted_average(_weight), raw=True)
...
class DPOIndicator(IndicatorMixin):
    ...
    def _run(self):
        min_periods = 0 if self._fillna else self._window
        self._dpo = (
            self._close.shift(int((0.5 * self._window) + 1), fill_value=self._close.mean())
...
class CCIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._cci = (
            typical_price - typical_price.rolling(self._window, min_periods=min_periods).mean()
        ) / (
            self._constant
            * typical_price.rolling(self._window, min_periods=min_periods).apply(_mad, True)
        )
...
def adx(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return ADXIndicator(high=high, low=low, close=close, window=window, fillna=fillna).adx()
...
def adx_pos(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return ADXIndicator(high=high, low=low, close=close, window=window, fillna=fillna).adx_pos()
...
def adx_neg(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return ADXIndicator(high=high, low=low, close=close, window=window, fillna=fillna).adx_neg()
...
def ichimoku_base_line(high, low, window1=9, window2=26, visual=False, fillna=False) -> pd.Series:
    """Kijun-sen (Base Line)
