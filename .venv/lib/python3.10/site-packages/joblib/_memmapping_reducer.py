...
def unlink_file(filename):
    ...
    for retry_no in range(1, NUM_RETRIES + 1):
        try:
            ...
            util.debug(
                "[ResourceTracker] tried to unlink {}, got " "PermissionError".format(filename)
            )
...
def _strided_from_memmap(
    filename, dtype, mode, offset, order, shape, strides, total_buffer_len, unlink_on_gc_collect
):
