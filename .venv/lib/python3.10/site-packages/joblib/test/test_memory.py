...
def test_memory_integration(tmpdir):
    ...
    for compress in (False, True):
        for mmap_mode in ("r", None):
            memory = Memory(
                location=tmpdir.strpath, verbose=10, mmap_mode=mmap_mode, compress=compress
            )
...
def test_memory_pickle_dump_load(tmpdir, memory_kwargs):
    ...
    compare(
        memory, memory_reloaded, ignored_attrs=set(["store_backend", "timestamp", "_func_code_id"])
    )
...
class TestCacheValidationCallback:
    ...
    def test_constant_cache_validation_callback(self, memory, consider_cache_valid):
        ...
        f = memory.cache(
            self.foo, cache_validation_callback=lambda _: consider_cache_valid, ignore=["d"]
        )
