...
def test_main_thread_renamed_no_warning(backend, monkeypatch):
    ...
    monkeypatch.setattr(
        target=threading.current_thread(), name="name", value="some_new_name_for_the_main_thread"
    )
...
def test_parallel_with_unpicklable_functions_in_args(
    ...
    check_subprocess_call(
        [sys.executable, code_file.strpath], timeout=10, stdout_regex=r"\[0, 1, 4, 9, 16\]"
    )
