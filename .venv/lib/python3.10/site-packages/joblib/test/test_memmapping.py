...
@with_multiprocessing
@skipif(
    (sys.platform != "win32") or (), reason="PermissionError only easily triggerable on Windows"
)
...
def test_pool_with_memmap(factory, tmpdir):
    ...
    try:
        ...
        p.map(
            inplace_double, [(a, (i, j), 1.0) for i in range(a.shape[0]) for j in range(a.shape[1])]
        )
        ...
        p.map(
            inplace_double, [(b, (i, j), 2.0) for i in range(b.shape[0]) for j in range(b.shape[1])]
        )
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    ...
    for _ in range(3):
        ...
        p = subprocess.Popen(
            [sys.executable, "-c", cmd], stderr=subprocess.PIPE, stdout=subprocess.PIPE, env=env
        )
        p.wait()
        out, err = p.communicate()
