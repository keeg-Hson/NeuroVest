...
def _format_load_msg(func_id, args_id, timestamp=None, metadata=None):
    ...
    try:
        if metadata is not None:
            args = ", ".join(
                ["%s=%s" % (name, value) for name, value in metadata["input_args"].items()]
            )
...
class MemorizedResult(Logger):
    ...
    def get(self):
        ...
        if self.verbose:
            msg = _format_load_msg(
                self.func_id, self.args_id, timestamp=self.timestamp, metadata=self.metadata
            )
        else:
            msg = None
        ...
        try:
            ...
            new_exc = KeyError(
                ...
                "It seems that this folder is corrupted : {}".format(
                    os.path.join(self.store_backend.location, self.func_id, self.args_id)
                )
            )
    ...
    def __repr__(self):
        return '{class_name}(location="{location}", func="{func}", ' 'args_id="{args_id}")'.format(
...
class Memory(Logger):
    ...
    def cache(
        self, func=None, ignore=None, verbose=None, mmap_mode=False, cache_validation_callback=None
    ):
