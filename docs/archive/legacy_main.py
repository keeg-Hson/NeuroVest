...
if not api_key:
    # raise ValueError("ERROR: API key not found. Check .env file location")
...
# ---***RECALCULATION OF FEATURE COLUMNS + FEATURE CONSISTENCY *CRITICAL* FOR VALID MODEL OUTPUT****
def calculate_technical_indicators(df):
    # Adds extracted moving averages, returns, RSI, and other technical metrics
...
def train_model(
...
):  # Trains model on extracted technical features
    if features is None:
        features = get_feature_list()
    # selection of feature and target (X and y variables respectively) from DataFrame
    X = df[features]  # Features used for model training
    ...
    joblib.dump(model, "models/market_crash_model.pkl")  # Saves pre-trained model
    return model
