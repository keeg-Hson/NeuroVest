...
<<<<<<< HEAD
        ...
        if pred_values.issubset({0, 1, 2}) and 2 in pred_values:
            # Already in 3-class format, skip mapping
...
<<<<<<< HEAD
    ...
    parser.add_argument(
        ...
        type=str,
        default="SPY",
        help="Asset ticker to backtest (e.g., SPY, QQQ, BTC/USDT). Default: SPY"
    )
    parser.add_argument(
        "--asset-group",
        type=str,
        choices=["equity", "crypto", "bond", "commodity", "all"],
        help="Backtest all assets in a group (equity/crypto/bond/commodity/all)"
    )
    parser.add_argument(
        "--compare",
        action="store_true",
...
<<<<<<< HEAD
    ...
    if args.compare or args.asset_group:
        ...
        if args.asset_group:
            if args.asset_group == "all":
                assets = [a.ticker for a in mgr.get_all_assets()]
            else:
                # Filter by asset_type
                assets = [a.ticker for a in mgr.get_all_assets() if a.asset_type == args.asset_group]

            if not assets:
                print(f"No assets found for group: {args.asset_group}")
                exit(1)
        else:
            # Use specific assets for comparison
            assets = [args.asset]
        ...
        print(f"Group: {args.asset_group if args.asset_group else 'Custom'}")
        print(f"{'=' * 80}\n")

        results = []
        for i, asset in enumerate(assets, 1):
            print(f"\n[{i}/{len(assets)}] Backtesting {asset}...")
            print(f"{'-' * 60}")

            # Check if data file exists
            asset_filename = asset.replace('/', '_') + '_1d.csv'
            asset_path = Path('data_cache') / asset_filename
            ...
            if asset == 'SPY':
                ...
                if main_spy_path.exists():
                    asset_path = main_spy_path

            if not asset_path.exists():
                print(f"   ⚠️  Data file not found: {asset_path}, skipping")
                continue

            try:
                trades, m, sim = run_backtest(asset=asset, **kwargs)
                results.append({
                    'Asset': asset,
                    'Trades': m['trades'],
                    ...
                    'Avg Long': m.get('avg_long', 0),
                    'Avg Short': m.get('avg_short', 0),
                })
                print(f"   ✓ {asset}: {m['total_return']:.2%} return, {m['sharpe']:.2f} Sharpe")
            except Exception as e:
                print(f"   ✗ Error: {e}")
        ...
        if results:
            ...
            print(f"\n{'=' * 80}")
            print("BACKTEST COMPARISON RESULTS (sorted by Sharpe)")
            print(f"{'=' * 80}\n")
            ...
            print(f"Best performer:       {df_results.iloc[0]['Asset']} ({df_results.iloc[0]['Total Return']:.2%} return)")
            print(f"Worst performer:      {df_results.iloc[-1]['Asset']} ({df_results.iloc[-1]['Total Return']:.2%} return)")
            print(f"Average return:       {df_results['Total Return'].mean():.2%}")
            print(f"Average Sharpe:       {df_results['Sharpe'].mean():.2f}")
            print(f"Median return:        {df_results['Total Return'].median():.2%}")
            print(f"{'=' * 80}\n")

            # Save results
            output_path = Path('logs') / f"backtest_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            df_results.to_csv(output_path, index=False)
            print(f"📊 Results saved to: {output_path}\n")
        else:
            print("\n⚠️  No successful backtests to compare")

        exit(0)

    # Add asset parameter from CLI
    if args.asset:
