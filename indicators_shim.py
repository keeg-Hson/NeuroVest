...
def rsi(df: pd.DataFrame, close="close", length=14, col="rsi"):
    df[col] = RSIIndicator(close=df[close], window=length).rsi()
    return df


def sma(df: pd.DataFrame, close="close", length=50, col="sma"):
    df[col] = SMAIndicator(close=df[close], window=length).sma_indicator()
    return df


def ema(df: pd.DataFrame, close="close", length=21, col="ema"):
    df[col] = EMAIndicator(close=df[close], window=length).ema_indicator()
    return df
...
def stoch(
    ...
    st = StochasticOscillator(
        high=df[high], low=df[low], close=df[close], window=k, smooth_window=d
    )
    df[k_col], df[d_col] = st.stoch(), st.stoch_signal()
    return df
